...
bool rootToNodePath(TreeNode *root, vector<int> &ans, int &node)
{
    if (root == nullptr)
        return false;
    ...
    if (rootToNodePath(root->left, ans, node) == true)
    {
        return true;
    }
    if (rootToNodePath(root->right, ans, node) == true)
        return true;

    ans.pop_back();
    return false;
}
vector<int> findNodeToRootPath(TreeNode *root, int node)
{
    vector<int> ans;
    if (root == nullptr)
        return ans;
